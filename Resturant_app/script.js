...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const generateBill = () => {
    ...
    let billDetails = `Bill Details for ${table.name}:\n\n`;
    let totalCost = 0;

    table.orders.forEach((order, index) => {
      const itemCost = order.item.price * order.quantity;
      totalCost += itemCost;
      billDetails += `${index + 1}. ${order.item.name} - ${order.quantity} x Rs.${order.item.price.toFixed(2)} = Rs.${itemCost.toFixed(2)}\n`;
    });
  ...
  const showOrderDetails = (tableId) => {
        ...
        <tbody>
          ${table.orders.map((order, index) => `
            <tr>
              ...
              <td>
                <input type="number" value="${order.quantity}" min="1" class="quantity-input" data-item-id="${order.item.id}">
              </td>
              <td class="total-price">Rs.${(order.item.price * order.quantity).toFixed(2)}</td>
              <td>
                <button onclick="removeOrder(${table.id}, ${order.item.id})">Remove</button>
              </td>
            </tr>
          `).join("")}
        </tbody>
      </table>
      <button id="generateBillButton" style="margin-top: 10px;">Generate Bill</button>
    `;
    ...
    document.querySelectorAll(".quantity-input").forEach(input => {
      input.addEventListener("input", (event) => {
        updateOrderQuantity(event.target.dataset.itemId, event.target.value);
      });
    });
  ...
  const updateOrderQuantity = (menuItemId, quantity) => {
    const table = tables.find(table => table.id === currentTableId);
    const order = table.orders.find(order => order.item.id === parseInt(menuItemId));

    const previousQuantity = order.quantity;
    const newQuantity = parseInt(quantity);

    table.totalCost += (newQuantity - previousQuantity) * order.item.price;
    table.totalItems += (newQuantity - previousQuantity);

    order.quantity = newQuantity;

    // Update the total price for the item in the order details
    const orderDetailsDiv = document.getElementById("orderDetails");
    const orderRow = orderDetailsDiv.querySelector(`input[data-item-id="${menuItemId}"]`).closest("tr");
    orderRow.querySelector(".total-price").textContent = `Rs.${(order.item.price * newQuantity).toFixed(2)}`;

    renderTables(tables);
  };

  window.removeOrder = (tableId, menuItemId) => {
    const table = tables.find(table => table.id === tableId);
    const orderIndex = table.orders.findIndex(order => order.item.id === menuItemId);
  ...
  tableSearch.addEventListener("input", () => {
    ...
    clearTableSearch.style.display = searchValue ? "inline" : "none";
  });
  ...
  menuSearch.addEventListener("input", () => {
    ...
    clearMenuSearch.style.display = searchValue ? "inline" : "none";
  });
